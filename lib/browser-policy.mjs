...
export const browserPolicyHeaders = [
  ...cspHeaders(['/tools/english-dictionary'],
    `${toolCsp('connect-src', `${baseConnect} https://api.dictionaryapi.dev`)}; media-src 'self' https://api.dictionaryapi.dev`),
  ...
  ...cspHeaders([
    ...
    '/tools/robots-txt-analyzer', '/tools/robots-txt-validator',
    '/tools/robots-txt-editor', '/tools/robots-txt-checker',
    '/tools/sitemap-analyzer', '/tools/sitemap-extractor',
  ], toolCsp('connect-src', `${baseConnect} https:`)),
  // The current downloader uses /api/favicon. Retain direct provider
  // compatibility on these two tools only, including Google's redirect.
  ...cspHeaders([
    '/tools/batch-favicon-downloader', '/tools/images/favicon-grabber',
  ...
  ...cspHeaders([
    ...
    '/tools/markdown-preview', '/tools/markdown-editor',
