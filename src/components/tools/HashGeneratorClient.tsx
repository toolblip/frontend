    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-gray-100 text-sm focus:outline-none focus:border-green-500 placeholder-gray-600 resize-y font-mono"
        />
      </div>
        ...
        {ALGORITHMS.filter(algo => activeAlgos.has(algo)).map(algo => {
          ...
          return (
            <div key={algo} className="bg-gray-800 border border-gray-700 rounded-lg p-4">
              ...
              <p className={`font-mono text-sm break-all ${hash ? 'text-green-400' : 'text-gray-600'}`}>
                {hash || '—'}
              </p>
            </div>
          );
