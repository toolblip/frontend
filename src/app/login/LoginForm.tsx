...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://api.toolblip.com'}/api/auth/login`, {
        method: 'POST',
  ...
  return (
    <div className="max-w-md mx-auto px-4 sm:px-6 py-10 sm:py-16">
      ...
      <p className="mt-6 text-center text-sm text-gray-400">
        ...
        <Link href="/signup" className="text-green-400 hover:text-green-300 underline transition-colors">
          Sign up
        </Link>
      </p>
    </div>
