...
// ─── Icons ────────────────────────────────────────────────────────────────────
const Terminal = () => <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="4 17 10 11 4 5"/><line x1="12" y1="19" x2="20" y2="19"/></svg>;
...
const Json    = () => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="13" x2="15" y2="13"/><line x1="9" y1="17" x2="12" y2="17"/></svg>;
...
function Method({ m }: { m: 'GET' | 'POST' | 'DELETE' }) {
  ...
  const s = map[m];
  return <span style={{ fontFamily: 'var(--f-mono)', fontWeight: 700, fontSize: 11, padding: '4px 11px', borderRadius: 6, letterSpacing: '0.05em', background: s.bg, color: s.fg, flexShrink: 0 }}>{m}</span>;
}

// ─── Section label ─────────────────────────────────────────────────────────────
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.09em', color: t.fg3, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}>{children}</p>;
}

// ─── Code block ───────────────────────────────────────────────────────────────
function CodeBlock({ code, label }: { code: string; label?: React.ReactNode }) {
  return (
    <div style={{ position: 'relative' }}>
      {label && <SectionLabel>{label}</SectionLabel>}
      <CopyButton text={code} />
      <pre style={{ fontFamily: 'var(--f-mono)', background: t.codeBg, color: t.codeFg, borderRadius: 10, padding: '18px 22px', fontSize: 12.5, lineHeight: 1.8, overflowX: 'auto' as const, margin: label ? '0 0 0 0' : 0, tabSize: 2, whiteSpace: 'pre' as const }}>{code}</pre>
    </div>
  );
}
...
function ParamTable({ params }: { params: Param[] }) {
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse' as const, fontSize: 13, marginTop: 14 }}>
      <thead>
        <tr>
          {['Name', 'In', 'Type', 'Description'].map((h, i) => (
            <th key={h} style={{ textAlign: 'left' as const, padding: '8px 12px 8px 0', color: t.fg3, fontWeight: 600, fontSize: 10.5, textTransform: 'uppercase' as const, letterSpacing: '0.06em', borderBottom: `1px solid ${t.line}`, width: [160, 72, 100, 0][i] }}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
    ...
    <div id={id} style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 16, overflow: 'hidden', marginBottom: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderTop: `1px solid ${t.line}` }}>
        <div style={{ padding: '18px 22px', borderRight: `1px solid ${t.line}` }}>
          <CodeBlock code={curl} label={<><Terminal /> curl</>} />
        </div>
        <div style={{ padding: '18px 22px' }}>
          <CodeBlock code={response} label={<><Check /> Response</>} />
        </div>
      </div>
    </div>
  );
...
export default function ApiDocsPage() {
  return (
    <div style={{ minHeight: '100vh', background: t.bg, fontFamily: 'var(--f-sans)', color: t.fg0 }}>
      ...
      <div style={{ background: t.surface, borderBottom: `1px solid ${t.border}`, padding: '64px 0 52px' }}>
        <div style={{ maxWidth: 1080, margin: '0 auto', padding: '0 32px' }}>
          ...
          <p style={{ color: t.fg2, fontSize: 15.5, lineHeight: 1.75, margin: 0, maxWidth: 540 }}>
            Integrate Toolblip into your app. All endpoints return JSON over HTTPS with Bearer token authentication.
          </p>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' as const, marginTop: 24 }}>
            {[
              { icon: <Terminal />, label: 'Base:', value: BASE },
              { icon: <Lock />,     label: 'Auth:', value: 'Bearer token' },
              { icon: <Doc />,      label: 'Format:', value: 'application/json' },
            ].map(b => (
          ...
          <main>
            ...
            <section id="overview" style={{ marginBottom: 64 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                {[
                  { icon: <Shield />, label: 'Bearer token auth', desc: 'Simple token-based authentication for protected endpoints.' },
                  { icon: <Box />,    label: 'RESTful',           desc: 'Standard HTTP methods — GET, POST. Predictable and intuitive.' },
                  { icon: <Json />,   label: 'JSON responses',   desc: 'Every endpoint returns application/json. Always.' },
                ].map(item => (
                  <div key={item.label} style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 12, padding: '16px 18px' }}>
                    <div style={{ color: t.blue, marginBottom: 10 }}>{item.icon}</div>
            ...
            <section id="base-url" style={{ marginBottom: 64 }}>
              ...
              <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 12, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                <span style={{ fontSize: 10.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: t.greenFg, background: t.greenBg, padding: '2px 9px', borderRadius: 5, flexShrink: 0 }}>Active</span>
              ...
              <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 12, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontSize: 10.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: t.fg3, padding: '2px 9px', borderRadius: 5, flexShrink: 0 }}>Pending</span>
                <code style={{ fontFamily: 'var(--f-mono)', fontSize: 13.5, color: t.fg2 }}>{CUSTOM}</code>
              </div>
              <p style={{ color: t.fg3, fontSize: 12.5, marginTop: 12 }}>Use the Railway URL for now. Switch to the custom domain once SSL is ready.</p>
            </section>
                ...
                response={`// 200 OK
{
  "user": {
    ...
    "is_pro": true
            ...
            <section id="errors" style={{ marginBottom: 64 }}>
              ...
              <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 14, padding: '22px 24px' }}>
                {([
                  ...
                  <div key={err.code} style={{ display: 'grid', gridTemplateColumns: '52px 190px 1fr', gap: 10, paddingBottom: i === 6 ? 0 : 14, borderBottom: i === 6 ? 'none' : `1px solid ${t.line}`, alignItems: 'baseline' }}>
            ...
            {/* Quickstart callout */}
            <div style={{ background: t.accentBg, border: `1px solid ${t.accentBdr}`, borderRadius: 14, padding: '20px 24px', marginTop: 8 }}>
