...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://api.toolblip.com'}/api/auth/signup`, {
        method: 'POST',
  ...
  return (
    <div className="max-w-md mx-auto px-4 sm:px-6 py-10 sm:py-16">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="password-confirm" className="block text-sm text-gray-400 mb-1">Confirm password</label>
          <input
            id="password-confirm"
            type="password"
      ...
      <p className="mt-6 text-center text-sm text-gray-400">
        ...
        <Link href="/login" className="text-green-400 hover:text-green-300 underline transition-colors">
          Sign in
        </Link>
      </p>
    </div>
