...
export default function SignupForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(
        ...
        { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ email, password, password_confirmation: confirm }) }
      );
      if (res.ok) window.location.href = '/';
      else { const data = await res.json().catch(() => ({})); setError(data.message ?? 'Could not create account. Please try again.'); }
  ...
  return (
    <div className="max-w-md mx-auto px-4 sm:px-6 py-10 sm:py-16">
      ...
      <a href={`${process.env.NEXT_PUBLIC_API_URL || 'https://api.toolblip.com'}/api/auth/google`} className="flex items-center justify-center gap-3 w-full bg-white hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-900 dark:text-white font-medium py-2.5 px-4 rounded-lg transition-colors mb-6 border border-gray-300 dark:border-gray-700">
        <svg width="18" height="18" viewBox="0 0 18 18" xmlns="https://www.w3.org/2000/svg" aria-hidden="true">
