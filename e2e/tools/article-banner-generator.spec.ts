...
test.describe('Banner Generator tool', () => {
  ...
  test('renders a true 1200x630 preview and exports the same canvas as PNG', async ({ page }) => {
    ...
    const download = page.getByRole('link', { name: /Download PNG/i }).first();
  ...
  test('keeps non-content controls collapsible while content stays open', async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 1100 });
    await page.goto('/tools/og-image-generator');

    await expect(page.getByText('CONTENT', { exact: true })).toBeVisible();
    await expect(page.getByLabel('Banner title')).toBeVisible();
    await expect(page.getByLabel('Banner subtitle')).toBeVisible();

    for (const section of ['Background', 'Typography', 'Pattern overlay', 'Footer']) {
      const toggle = page.getByRole('button', { name: new RegExp(`${section} controls`, 'i') });
  ...
  test('exposes configurable resolution and ratio controls plus top and bottom downloads', async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 1100 });
    await page.goto('/tools/og-image-generator');

    await expect(page.getByLabel('Banner ratio')).toHaveValue('1200x630');
    await expect(page.getByLabel('Banner resolution')).toHaveValue('1200x630');
    await expect(page.getByText('Preview 1200 × 630 px')).toBeVisible();

    const downloads = page.getByRole('link', { name: /Download PNG/i });
    await expect(downloads).toHaveCount(2);
    await expect(downloads.first()).toHaveAttribute('href', /^data:image\/png;base64,/);
    await expect(downloads.nth(1)).toHaveAttribute('href', /^data:image\/png;base64,/);

    const preview = page.getByTestId('article-banner-preview');
    await expect(preview).toHaveJSProperty('width', 1200);
    await expect(preview).toHaveJSProperty('height', 630);

    await page.getByLabel('Banner ratio').selectOption('square');
    await expect(page.getByLabel('Banner resolution')).toHaveValue('1080x1080');
    await expect(page.getByText('Preview 1080 × 1080 px')).toBeVisible();
    await expect(preview).toHaveJSProperty('width', 1080);
    await expect(preview).toHaveJSProperty('height', 1080);

    await page.getByLabel('Banner resolution').selectOption('800x420');
    await expect(page.getByLabel('Banner ratio')).toHaveValue('custom');
    await expect(page.getByText('Preview 800 × 420 px')).toBeVisible();
    await expect(preview).toHaveJSProperty('width', 800);
    await expect(preview).toHaveJSProperty('height', 420);
  });
  ...
  test('exposes screenshot-style banner configuration controls and updates the export', async ({ page }) => {
    ...
    const patternOverlay = page.getByLabel('Pattern overlay', { exact: true });
