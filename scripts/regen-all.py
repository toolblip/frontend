...
for bf in batch_files:
    ...
    # Find all entries
    # Pattern: 'slug': { description: `...`, examples: [...], features: [...] }
    # We need to extract the full entry including nested braces
    
    # Simple approach: find 'slug': { and extract until the next entry or end
    entries = re.finditer(r"'([a-z0-9-]+)':\s*\{", content)
    for m in entries:
        slug = m.group(1)
        start = m.start()
        ...
        brace_count = 0
        pos = m.end() - 1  # Start at the opening brace
        ...
        # Parse the entry
        desc_match = re.search(r'description:\s*`([^`]+)`', entry_text)
        features_match = re.search(r'features:\s*\[([^\]]+)\]', entry_text)
        
        if desc_match and features_match:
            ...
            for ex in re.finditer(example_pattern, entry_text):
                title = ex.group(1)
                code = ex.group(2)
                note = ex.group(3) or ''
...
# Build output
out = []
out.append("""// AUTO-GENERATED by scripts/regen-all.py
