...
function getNextRuns(expr: string, count = 5): Date[] {
 ...
 return result.parsed ? computeNextRuns(result.parsed, count) : [];
}
...
export default function CronGeneratorClient() {
  ...
  return (
    <DeveloperGeneralFrame><>
      {mounted && (
        <div>
          <div className="tb-v2-tool-input-head">
            <span className="tb-v2-tool-label">Cron Expression</span>
            <code className="tb-v2-hash-stats">{expr}</code><ToolExampleClearActions onExample={() => applyPreset('0 9 * * 1-5')} onClear={() => applyPreset('    ')} />
          </div>
