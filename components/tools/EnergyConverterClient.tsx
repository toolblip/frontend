...
const UNITS = ['Joules', 'Kilojoules', 'Calories', 'Kilocalories', 'Watt-hours', 'Kilowatt-hours', 'Electronvolts'];
const FACTORS = [1, 1000, 4.184, 4184, 3600, 3.6e6, 1.602e-19];

export default function EnergyConverterClient() {
  const [value, setValue] = useState(1000);
  const [from, setFrom] = useState(0);
  const results = useMemo(() => {
    const joules = value * FACTORS[from];
    return UNITS.map((u, i) => ({ unit: u, value: joules / FACTORS[i] }));
  }, [value, from]);
      ...
      <div style={{ marginTop: '1rem' }}>
        {results.map(r => (
          <div key={r.unit} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 0.75rem',
            ...
            <span style={{ fontFamily: 'monospace' }}>{r.value.toExponential(4)}</span>
