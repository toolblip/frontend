...
function SharePopover({ toolName, channels, copied, onShare, onCopy, onClose, pageUrl, shortUrl, loading, showQr, onToggleQr, qrDataUrl }: { toolName: string; channels: ShareChannel[]; copied: boolean; onShare: (channel: string) => void; onCopy: () => void; onClose: () => void; pageUrl: string; shortUrl: string; loading: boolean; showQr: boolean; onToggleQr: () => void; qrDataUrl: string }) {
  return (
    <div
      role="dialog"
    ...
    >
      ...
      <div className="flex gap-2 p-3">
        {channels.map((link) => (
          <a
            ...
            className="flex flex-1 flex-col items-center justify-center gap-1.5 rounded-xl border border-gray-200/80 bg-white px-2 py-3 transition hover:-translate-y-0.5 hover:border-gray-300 hover:bg-white hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-red-400 cursor-pointer dark:border-gray-700 dark:bg-slate-900/80 dark:hover:border-gray-600 dark:hover:bg-slate-900"
          >
            {link.icon}
            <span className="text-[10px] font-medium text-gray-500 dark:text-gray-400">{link.label.replace("Share on ", "")}</span>
          </a>
        ))}
      </div>
      ...
      <div className="border-t border-gray-100 px-3 pb-3 pt-2 dark:border-gray-800">
        ...
        <button
          type="button"
          onClick={onToggleQr}
          className="mt-2 w-full flex items-center justify-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-50 cursor-pointer dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800"
        >
        ...
        {/* QR Code display */}
        {showQr && (
          <div className="mt-2 flex flex-col items-center rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
            {qrDataUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={qrDataUrl} alt="QR Code" className="h-[160px] w-[160px] rounded-lg" style={{ imageRendering: "pixelated" }} />
            ) : (
            ...
            <p className="mt-2 text-[10px] text-gray-400 dark:text-gray-500">Scan to open link</p>
          </div>
        )}
      </div>
    </div>
    ...
    return [
      {
        label: "Share on X",
        channel: "x",
        url: `https://x.com/intent/tweet?${new URLSearchParams({ text, url: shortUrl }).toString()}`,
        icon: (
      ...
      },
      {
        ...
        channel: "facebook",
        url: `https://www.facebook.com/sharer/sharer.php?${new URLSearchParams({ u: shortUrl }).toString()}`,
        icon: (
      ...
      },
      {
        ...
        channel: "linkedin",
        url: `https://www.linkedin.com/sharing/share-offsite/?${new URLSearchParams({ url: shortUrl }).toString()}`,
        icon: (
          <svg className="h-6 w-6 text-[#0A66C2]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      ...
      },
      {
        ...
        channel: "reddit",
        url: `https://reddit.com/submit?${new URLSearchParams({ url: shortUrl, title: text }).toString()}`,
        icon: (
          <svg className="h-6 w-6 text-[#FF4500]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      ...
      },
      {
        label: "Share on HN",
        channel: "hn",
        url: `https://news.ycombinator.com/submitlink?${new URLSearchParams({ u: shortUrl, t: toolName }).toString()}`,
        icon: (
          <svg className="h-6 w-6 text-[#FF6600]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M0 0v24h24V0H0zm12.7 13.5v6.3h-1.5v-6.3L7.4 6h1.7l3 5.5L15.1 6h1.7l-4.1 7.5z" />
          </svg>
        ),
      },
    ];
  }, [pageUrl, toolName, shortUrl]);
  ...
  return (
    <div data-testid="tool-engagement-bar" className="relative flex w-full flex-wrap items-center gap-3" aria-label={`${toolName} engagement stats`}>
      ...
      >
        ...
        {shareOpen && <SharePopover toolName={toolName} channels={shareLinks} copied={copied} pageUrl={pageUrl} shortUrl={shortUrl} loading={shortUrlLoading} showQr={showQr} onToggleQr={() => setShowQr(!showQr)} qrDataUrl={qrDataUrl} onShare={(channel) => void recordShare(channel)} onCopy={copyLink} onClose={() => { setShareOpen(false); setShowQr(false); }} />}
      </div>
