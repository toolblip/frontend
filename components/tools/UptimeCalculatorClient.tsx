...
export default function UptimeCalculatorClient() {
  ...
  const downtime = calcDowntime(Number(sla), period);
  ...
  const slaDecimal = (100 - Number(sla)).toFixed(Math.min(6, slaDecimalPlaces + 1));
  const periods: Period[] = ['year', 'month', 'week', 'day', 'hour'];

  return (<UtilityDesignLayout>
    <div>
      <ToolExampleClearActions onExample={() => { setSla('99.9'); setPeriod('day'); }} onClear={() => { setSla(''); setPeriod('year'); }}/>
      ...
      {!sla || !Number.isFinite(Number(sla)) || Number(sla) < 0 || Number(sla) > 100 ? <p role="alert">Enter an SLA from 0 to 100%.</p> : <>
      ...
      <div className="tb-v2-tool-output-body">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 8 }}>
            {periods.map(p => {
              const d = calcDowntime(Number(sla), p);
              const label = d < 1 ? `${(d * 60).toFixed(1)} sec` : d < 60 ? `${d.toFixed(2)} min` : `${(d / 60).toFixed(2)} hrs`;
              return (
      ...
      <p>Month = 30 days; year = 365 days.</p>
    </div>
  </UtilityDesignLayout>
  );
}
