...
export default function CombineImagesClient() {
  ...
  const canPreview = items.length >= MIN_COMBINE_IMAGES && !actionError && !loading;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void (async () => {
        ...
        if (!context) throw new Error('Could not start the image preview in this browser. Try again.');
        canvas.width = combinePlan.width;
        canvas.height = combinePlan.height;
        context.fillStyle = '#ffffff';
        context.fillRect(0, 0, combinePlan.width, combinePlan.height);
        await Promise.all(items.map((item, index) => new Promise<void>((resolve, reject) => {
          ...
          image.onload = () => {
            ...
            try {
              const placement = combinePlan.placements[index];
        ...
        const output = await canvasToVerifiedImageResult(
          canvas,
          `combined-${combinePlan.width}x${combinePlan.height}.${extension}`,
          mimeType,
        );
        if (!outputActionsRef.current.isCurrentAttempt(attemptId)) {
        ...
        setExportError(err instanceof Error ? err.message : 'The image preview could not be exported. Try again.');
      }).finally(() => {
  ...
  }, [canPreview, exampleLoading, items, combinePlan, layout, spacing, retryPreview]);
    ...
    <div className="tb-v2-tool-card">
      ...
      <div className={`tb-v2-tool-output-body ${styles.toolBody}`}>
        ...
        >
          ...
          <span className="tb-v2-dropzone-hint">Add 2–12 images</span>
        </button>
        ...
        {items.length > 0 && (
          <div className={styles.sourceSection}>
            <div className={styles.statsGrid}>
              ...
              <div className={`tb-v2-stat-pill ${styles.statPill}`}><span className={`tb-v2-stat-pill-val ${styles.statValue}`}>{combinePlan.width} x {combinePlan.height}</span><span className={`tb-v2-stat-pill-lbl ${styles.statLabel}`}>Output size</span></div>
            </div>
            ...
            <div className="tb-v2-mode-tabs" role="group" aria-label="Combine layout">
              {(['horizontal', 'vertical', 'grid'] as CombineLayoutMode[]).map((mode) => (
                <button
                  key={mode}
                  type="button"
                  onClick={() => { if (layout === mode) return; setExportError(''); clearResult(); setLayout(mode); }}
            ...
            <details className={styles.advancedHelp}>
              ...
              <p>{sourceLimitText} {boundsText} Background is white. Images keep their original pixel sizes.</p>
            </details>
            ...
            {exporting && <div className="tb-v2-banner tb-v2-banner-info" role="status">Updating preview…</div>}
            ...
            <p className="tb-v2-dropzone-hint">Drag images to change their order.</p>
        ...
        {items.length === 0 && <p className="tb-v2-empty">Add at least two images to see the result.</p>}
