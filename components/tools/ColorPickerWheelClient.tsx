...
export default function ColorPickerWheelClient() {
  ...
  const loadExample = () => setColor('#22c55e');
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="tb-v2-section" style={{display:"flex",flexDirection:"column",gap:16,padding:"16px 20px"}}>
        <input type="color" value={color} onChange={e => setColor(e.target.value)} className="w-full h-40 rounded-full cursor-pointer border-0" />
        ...
        <div className="rounded-full h-24 w-24 mx-auto" style={{ backgroundColor: color }} />
      </div>
    </div>
  );
}
