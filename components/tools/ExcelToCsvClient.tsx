...
export default function ExcelToCsvClient() {
  ...
  const currentCsv = sheets ? gridToCsv(sheets[sheetIndex].grid) : '';
  ...
  const downloadCsv = () => {
    ...
    a.href = url;
    a.download = `${(fileName.replace(/\.xlsx$/i, '') || 'sheet')}-${sheets[sheetIndex].name}.csv`;
    a.click();
    ...
    <div className="tb-v2-tool-card">
      ...
      {sheets && (
        <div style={{ padding: '0 20px 20px' }}>
          ...
          {sheets[sheetIndex].grid.length > 200 && (
            <div className="tb-v2-banner" style={{ marginTop: 8 }}>Preview limited to the first 200 rows. The downloaded CSV includes every row.</div>
          )}

          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <button type="button" onClick={downloadCsv} className="tb-v2-btn tb-v2-btn-primary">Download CSV</button>
            <button type="button" onClick={copyCsv} className="tb-v2-btn-sm">{copied ? 'Copied' : 'Copy CSV'}</button>
          </div>
        </div>
      )}

      {!sheets && !error && !loading && <p className="tb-v2-empty" style={{ margin: '0 20px 20px' }}>Upload an .xlsx file to convert it to CSV.</p>}
    </div>
  );
}
