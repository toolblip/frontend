...
export default function CidrCalculatorClient() {
  ...
  const InfoRow = ({ label, value, copy }: { label: string; value: string; copy?: string }) => (
  ...
  return (
    <div>
      <div className="tb-v2-tool-input-head"><span className="tb-v2-tool-label">CIDR Notation</span></div>
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        <input type="text" value={cidr} onChange={(e) => setCidr(e.target.value)} placeholder="192.168.1.0/24"
          className="tb-v2-tool-textarea" style={{ flex: 1, fontFamily: 'var(--f-mono)' }} aria-label="CIDR input" />
        <button type="button" onClick={calculate} className="tb-v2-copy-btn">Calculate</button>
      </div>
