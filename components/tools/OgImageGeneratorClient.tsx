...
export default function OgImageGeneratorClient() {
  ...
  useEffect(() => {
    ...
    const drawBanner = async () => {
      ...
      ctx.fillStyle = isDarkBg ? '#ffffff' : '#1a1a2e';
      ctx.font = `600 ${titleFontSize}px Inter, Arial, sans-serif`;
      ...
      ctx.fillStyle = isDarkBg ? 'rgba(255,255,255,0.78)' : 'rgba(26,26,46,0.55)';
      const subY = titleY + titleFontSize * 0.65;
      ctx.fillText(subtitle, textX, subY, maxTextWidth);

      ctx.textAlign = 'left';
      ctx.textBaseline = 'alphabetic';
      ctx.font = '600 28px Inter, Arial, sans-serif';
      ctx.fillStyle = isDarkBg ? 'rgba(255,255,255,0.78)' : 'rgba(26,26,46,0.5)';
      ...
      if (isFreeUser) {
        ...
        ctx.fillStyle = isDarkBg ? '#ffffff' : 'rgba(26,26,46,0.35)';
        ctx.textAlign = 'right';
        ctx.fillText('Generated by toolblip.com', WIDTH - 20, HEIGHT - 14);
        ctx.restore();
      }
      ...
      <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            {isFreeUser && (
              <span className="absolute bottom-3 right-3 z-10 rounded-md bg-black/40 px-2 py-1 text-[10px] text-white/70 backdrop-blur-sm">
                Generated by toolblip.com
              </span>
            )}
            <canvas
              ref={undefined}
