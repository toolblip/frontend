...
function tipsFor(ratio: number, aa: boolean, aaa: boolean): string[] {
  if (aaa) return ['Excellent contrast', 'Suitable for all text sizes'];
  if (aa) return ['Good contrast for normal text', 'Consider increasing further for AAA'];
  if (ratio >= 3) return ['Only suitable for large text (18pt+)', 'Increase contrast for body copy'];
...
export default function ContrastCheckerEnhancedClient() {
  ...
  const [result, setResult] = useState<{ ratio: number; aa: boolean; aaa: boolean; tips: string[] } | null>(null);
  ...
  const handleCheck = async () => {
    ...
    const aa = ratio >= 4.5;
    const aaa = ratio >= 7;
    setResult({ ratio, aa, aaa, tips: tipsFor(ratio, aa, aaa) });
    setLoading(false);
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="tb-v2-tool-input-head">
        <span className="tb-v2-tool-label">Contrast Checker Enhanced</span>
      ...
      <div className="tb-v2-section" style={{display:"flex",flexDirection:"column",gap:16,padding:"16px 20px"}}>
        ...
        {result && (
          <div className="bg-gray-50 rounded-xl p-4 flex flex-col gap-1">
            ...
            <ul className="text-sm list-disc pl-5 mt-1">{result.tips.map((t, i) => <li key={i}>{t}</li>)}</ul>
          </div>
        )}
      </div>
    </div>
