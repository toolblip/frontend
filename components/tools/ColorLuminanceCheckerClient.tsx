...
export default function ColorLuminanceCheckerClient() {
  const [colors, setColors] = useState<{ hex: string; name: string }[]>(DEFAULT_COLORS);
  ...
  const canClear =
    JSON.stringify(colors) !== JSON.stringify(DEFAULT_COLORS) || newHex.length > 0 || newName.length > 0;
    ...
    <div className="tb-v2-tool-card">
      <div className="tb-v2-tool-input-head" style={{ borderBottom: '1px solid var(--line)' }}>
        ...
        <ToolExampleClearActions
          onExample={() => setColors(EXAMPLE_COLORS)}
          onClear={clearAll}
          canClear={canClear}
        />
      </div>
      ...
      <div className="tb-v2-section" style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 20 }}>
              ...
              <div key={hex} className="flex items-center gap-4 p-3 rounded border">
                ...
                <div
                  className={`px-2 py-1 rounded text-xs font-medium ${
                    wcag === 'light' ? 'bg-green-100 text-green-800' : 'bg-gray-800 text-white'
                  }`}
                >
        ...
        <div className="p-4 bg-blue-50 rounded border border-blue-200 text-sm">
          <strong>Relative luminance</strong> (WCAG) determines whether light or dark text should be used on a
          background. Values above 17.9% are treated as light backgrounds.
        </div>
      </div>
    </div>
  );
