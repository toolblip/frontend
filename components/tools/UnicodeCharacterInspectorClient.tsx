...
function inspectChars(input: string) {
  return [...input].map((char) => {
    ...
    const utf8 = new TextEncoder()
      .encode(char)
      .reduce((a, b) => a + b.toString(16).toUpperCase().padStart(2, '0') + ' ', '')
      .trim();
    return {
      char,
...
export default function UnicodeCharacterInspectorClient() {
  ...
  const results = useMemo(() => (input ? inspectChars(input) : []), [input]);
  ...
  return (
    <div className="tb-v2-tool-card">
      ...
      <div className="tb-v2-tool-output-body">
        {results.length === 0 ? (
          <div className="tb-v2-empty">Type characters above — details update as you type</div>
        ) : (
          results.map((r, i) => (
            <div
              key={i}
            ...
            >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 6, fontSize: 13 }}>
                <div>
                  <span style={{ color: 'var(--tb-text-secondary)' }}>Code point:</span>{' '}
                  <code>{r.codePoint}</code>
                </div>
                <div>
                  <span style={{ color: 'var(--tb-text-secondary)' }}>Hex:</span> <code>{r.hex}</code>
                </div>
                <div>
                  <span style={{ color: 'var(--tb-text-secondary)' }}>Decimal:</span>{' '}
                  <code>{r.decimal}</code>
                </div>
                <div>
                  <span style={{ color: 'var(--tb-text-secondary)' }}>Binary:</span> <code>{r.binary}</code>
                </div>
                <div>
                  <span style={{ color: 'var(--tb-text-secondary)' }}>UTF-8:</span> <code>{r.utf8}</code>
                </div>
                <div>
                  <span style={{ color: 'var(--tb-text-secondary)' }}>HTML:</span> <code>{r.html}</code>
                </div>
              </div>
            </div>
          ))
