...
export default function ColorShadeTintsClient() {
  ...
  // Tints (toward white) and shades (toward black)
  const tints = rgb
  ...
  return (
    <div className="tb-v2-tool-card">
      ...
      <div className="tb-v2-section" style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: 20 }}>
        ...
        <div>
          <div className="text-xs text-gray-500 mb-2">Tints (lighter)</div>
        ...
        <div>
          <div className="text-xs text-gray-500 mb-2">Shades (darker)</div>
          <div className="flex gap-2">
            {shades.map((t, i) => (
              <button
                key={`s-${i}`}
                type="button"
                onClick={() => copy(`shade-${i}`, t)}
                className="flex-1 h-16 rounded-lg flex items-end justify-center pb-1"
                style={{ backgroundColor: t }}
              >
                <span className="text-xs font-mono" style={{ color: i > 1 ? '#fff' : '#000' }}>
                  {copied === `shade-${i}` ? 'Copied' : t}
                </span>
              </button>
            ))}
          </div>
