...
export default function EpsToPngClient() {
  ...
  const downloadPng = () => {
    ...
    canvas.toBlob(blob => {
      ...
      a.href = url;
      a.download = `${fileName.replace(/\.eps$/i, '') || 'bounding-box'}.png`;
      a.click();
      ...
      {info && (
        <div style={{ padding: '0 20px 20px' }}>
          <div className="tb-v2-banner" style={{ marginBottom: 16 }}>
            ...
            Below is real metadata extracted from the file, plus a bounding-box-accurate reference image with a
            transparent background you can download as PNG. For full rasterization, use Ghostscript, Adobe
            Illustrator, or macOS Preview.
          </div>
          ...
          <canvas ref={canvasRef} style={{ border: '1px solid var(--line)', maxWidth: '100%', background: 'repeating-conic-gradient(#e5e7eb 0% 25%, transparent 0% 50%) 50% / 16px 16px' }} />

          <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
            <button type="button" onClick={downloadPng} className="tb-v2-btn tb-v2-btn-primary">Download Bounding-Box Preview as PNG</button>
            {info.previewBytes && (
