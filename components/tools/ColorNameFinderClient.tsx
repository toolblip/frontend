  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-4 items-end">
        <div>
          <label className="tb-v2-tool-label" style={{ marginBottom: 8, display: 'block' }}>Pick a Color</label>
          <input type="color" value={color} onChange={e => setColorValue(e.target.value)} className="rounded-lg cursor-pointer border-2 border-gray-200" style={{ width: 96, height: 96 }} />
        </div>
      ...
      {rgb && hsl && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <button type="button" onClick={() => copy('rgb', `rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`)} className="bg-gray-50 rounded p-3 text-left">
            <span className="text-gray-500">RGB</span>
            <div className="font-mono font-medium">{copied === 'rgb' ? 'Copied' : `${rgb.r}, ${rgb.g}, ${rgb.b}`}</div>
          </button>
          <button type="button" onClick={() => copy('hsl', `hsl(${hsl.h}, ${hsl.s}%, ${hsl.l}%)`)} className="bg-gray-50 rounded p-3 text-left">
      ...
      {variations.length > 0 && (
        <div>
          <div className="tb-v2-tool-label" style={{ marginBottom: 8 }}>Variations</div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {variations.map(v => (
              <button
                key={v.name}
                type="button"
                onClick={() => copy(v.name, v.hex.toUpperCase())}
                className="text-center"
              >
                <div className="h-16 rounded-lg border mb-1" style={{ backgroundColor: v.hex }} />
                <div className="text-xs">{v.name}</div>
                <div className="text-xs font-mono text-gray-500">{copied === v.name ? 'Copied' : v.hex.toUpperCase()}</div>
              </button>
            ))}
          </div>
        </div>
