...
import PurchaseAgreement from './PurchaseAgreementGeneratorClient';
...
function mount() {
  ...
  const flush = () => {
    ...
    do {
      ...
      hooks.index = 0; hooks.dirty = false; tree = PurchaseAgreement();
...
const fields = [
  ['Buyer Name', 'Updated buyer'], ['Seller Name', 'Updated seller'], ['Price', '500'],
  ['Delivery Date', '2026-10-01'], ['Governing State', 'Oregon'],
  ['Item Description', 'Updated item'], ['Payment Terms', 'Payment on delivery'],
];
let downloads: Blob[];
beforeEach(() => {
...
describe('purchase agreement PDF invalidation', () => {
  ...
  it.each(fields)('editing %s clears an existing actual PDF encoding error', async (label, value) => {
    const view = mount(); view.change('Buyer Name', 'বাংলা');
    await view.export(); view.flush();
    expect(view.alerts().join(' ')).toContain('Download TXT');
    view.change(label, value);
    expect(view.alerts()).toEqual([]);
  });
  it('ignores a late real encoding error after a payment terms edit', async () => {
    const view = mount(); view.change('Buyer Name', 'বাংলা');
    const release = holdExport(), pending = view.export();
    view.change('Payment Terms', 'Revised while exporting');
    release(); await pending; view.flush();
  ...
  it('allows the current document to export after a stale operation finishes', async () => {
    ...
    view.change('Item Description', 'Current item after revision');
    release(); await pending; view.flush();
    exportGate.wait = Promise.resolve();
    await view.export(); view.flush();
    expect(downloads).toHaveLength(1);
    expect(view.preview()).toContain('Current item after revision');
