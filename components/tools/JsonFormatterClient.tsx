...
export default function JsonFormatterClient() {
  ...
  const { copied, copy, reset: setCopied, copyError } = useDataClipboard(result);
    ...
    <div>
      ...
      <div className="tb-v2-tool-input-head" style={{flexWrap:"wrap",gap:8}}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
          <ToolExampleClearActions onExample={() => { setInput(EXAMPLES[0].data); setShowExamples(true); }} onClear={() => { setInput(''); setCopied(false); setShowExamples(false); setMode('format'); setIndent(2); }} />
      ...
      <div className="tb-v2-tool-output-body" style={{maxWidth:"100%",overflowX:"auto"}}>
        {error ? (
          ...
          <pre aria-label="Output" className="tb-v2-tool-pre" style={{whiteSpace:"pre-wrap",overflowWrap:"anywhere"}}>{result || ' - '}</pre>
        )}
      </div>
    </div>
  );
