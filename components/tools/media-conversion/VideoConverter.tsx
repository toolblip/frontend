...
export default function VideoConverter({ subtitles = false }: { subtitles?: boolean }) {
  ...
  return <div className="tb-v2-section" style={{ display: 'grid', gap: 12, minWidth: 0 }}>
    <div className="tb-v2-tool-input-head" style={{ flexWrap: 'wrap', gap: 8 }}><span className="tb-v2-tool-label">{subtitles ? 'Burn subtitles into video' : 'Cut video'}</span><ToolExampleClearActions onExample={example} onClear={clear} /></div>
    <p>MP4/MOV, WebM or MKV if your browser can decode it. Maximum 30 MB, 2,073,600 pixels and 60 seconds per export. Recording runs in real time; keep this tab visible. Output is re-encoded WebM with audio, not a lossless cut. Cuts have frame-level timing tolerance.</p>
