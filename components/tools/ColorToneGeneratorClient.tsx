...
export default function ColorToneGeneratorClient() {
  ...
  const tones = rgb
    ? [1, 0.85, 0.7, 0.55, 0.4, 0.25].map((f) => {
          ...
          <div className="flex gap-2">
            {tones.map((t, i) => (
              <button
                key={i}
                type="button"
                onClick={() => copy(`tone-${i}`, t)}
                className="flex-1 h-16 rounded-lg flex items-end justify-center pb-1"
                style={{ backgroundColor: t }}
              >
                <span className="text-xs font-mono" style={{ color: i > 2 ? '#fff' : '#000' }}>
                  {copied === `tone-${i}` ? 'Copied' : t}
                </span>
              </button>
            ))}
          </div>
